/**
 * Rebuild Pristine Master Demo Data (100% Clean, Authentic, Zero Prototype Artefacts)
 */
...
// 1. Text for 112/VBHN-VPQH with full 35 Articles
const HTML_112_VBHN = `<div class="document-full-body legal-decree30-layout font-serif text-slate-900 leading-relaxed">
  ...
  <div class="space-y-6 text-justify text-sm sm:text-base">
    ...
    <div id="dieu-9" class="space-y-2">
      ...
      <p class="indent-6">2. Trường hợp cá nhân kinh doanh nhiều ngành, nghề thì áp dụng tỷ lệ thuế theo từng ngành, nghề. Trường hợp không xác định được doanh thu theo từng ngành, nghề thì áp dụng tỷ lệ thuế của ngành, nghề có tỷ lệ thuế cao nhất.</p>
    </div>
    ...
    <div id="dieu-21" class="space-y-2">
      ...
      <p class="indent-6">1. Thu nhập tính thuế đối với thu nhập từ tiền lương, tiền công là thu nhập chịu thuế quy định tại Điều 11 của Luật này trừ các khoản đóng bảo hiểm xã hội, bảo hiểm y tế, bảo hiểm thất nghiệp, quỹ hưu trí tự nguyện, các khoản giảm trừ quy định tại Điều 19 và Điều 20 của Luật này.</p>
...
// 2. Clean documents
const PRISTINE_DOCS: LegalDocument[] = [
  {
  ...
  },
  {
    ...
    updated_at: new Date().toISOString(),
    html_content: baseDocs.find((d: any) => d.document_number === '132/2020/NĐ-CP')?.html_content || '',
    files: [
      {
  ...
  },
  {
    ...
    updated_at: new Date().toISOString(),
    html_content: baseDocs.find((d: any) => d.document_number === '125/2020/NĐ-CP')?.html_content || '',
    files: [
      {
...
async function run() {
  console.log(`=== REBUILDING PRISTINE CLEAN DEMO DATA (${PRISTINE_DOCS.length} STATUTES) ===\n`);
  ...
  const outputCode = `// PACO LegalBook - Master Authentic Legal Database (100% Clean Pristine Corpus)
...
export const DEMO_DOCUMENTS: LegalDocument[] = ${JSON.stringify(PRISTINE_DOCS, null, 2)};
...
`;
  ...
  console.log(`Successfully wrote pristine ${DEMO_DATA_PATH} with ${PRISTINE_DOCS.length} verified documents.`);
  ...
  const supabaseDocs = PRISTINE_DOCS.map(d => ({
    id: d.id,
    title: d.title,
  ...
  if (upsertErr) {
    ...
    console.log(`✅ Upserted ${PRISTINE_DOCS.length} pristine authentic statutes to Supabase.`);
  }

  // Also sync document_files
  const allFiles = PRISTINE_DOCS.flatMap(d => d.files || []);
  await supabase.from('document_files').upsert(allFiles, { onConflict: 'id' });
  console.log(`✅ Upserted ${allFiles.length} file attachments to Supabase.`);
}
