...
const newRelations = [
  { source_document_id: 'doc-tt-99-ketoan-2025', target_document_id: 'doc-tt-200-2014', relation_type: 'thay_the', notes: 'Thay thế Chế độ kế toán DN Thông tư 200/2014/TT-BTC từ ngày 01/01/2026' },
  ...
  { source_document_id: 'doc-nd-70-hoadon-2025', target_document_id: 'doc-nd-123-2020', relation_type: 'sua_doi', notes: 'Sửa đổi NĐ 123/2020 về hóa đơn điện tử máy tính tiền' },
...
async function run() {
  ...
  for (const meta of rawFileMetadata) {
    const matchedFile = actualFiles.find(f => f.includes(meta.fileNameMatch)) || meta.id;
...
<p>Số hiệu: <strong>${meta.document_number}</strong> do <strong>${meta.issuing_body}</strong> ban hành.</p>
<p>Ngày ban hành: ${meta.issued_date} — Ngày hiệu lực: ${meta.effective_date}.</p>
<h3>1. Tóm tắt nội dung chính</h3>
<p>${meta.summary_main}</p>
<h3>2. Điểm mới nổi bật</h3>
<p>${meta.summary_new_points || 'Văn bản hướng dẫn nghiệp vụ mới.'}</p>
<h3>3. Tác động kế toán & kiểm toán</h3>
<p>${meta.summary_accounting_impact || 'Kiểm toán viên đối chiếu tuân thủ theo quy định hiện hành.'}</p>
<div class="bg-blue-50 border border-blue-200 rounded p-4 mt-6">
  ...
  <p class="mt-2">
    <a href="/documents/${encodeURIComponent(matchedFile)}" target="_blank" download class="inline-block px-3 py-1.5 bg-blue-700 text-white rounded text-xs font-semibold hover:bg-blue-800">
      ...
      // Append download box at end of html
  ...
  <a href="/documents/${encodeURIComponent(matchedFile)}" target="_blank" download class="inline-block px-3 py-1.5 bg-blue-700 text-white rounded text-xs font-semibold hover:bg-blue-800">
    ...
    const docObj = {
      ...
      summary_audit_impact: meta.summary_audit_impact || 'Kiểm toán viên đối chiếu tuân thủ theo quy định hiện hành.',
  ...
  console.log(`Successfully generated ${newDocs.length} PACO documents with HTML contents!`);

  // Write out as module export for demo-data.ts
  const outputCode = `// Generated AUDIT PACO Data
import type { LegalDocument } from '@/types';

export const PACO_DOCUMENTS: Partial<LegalDocument>[] = ${JSON.stringify(newDocs, null, 2)};
export const PACO_CATEGORY_LINKS: { document_id: string; category_id: string; is_primary: boolean }[] = ${JSON.stringify(newLinks, null, 2)};
export const PACO_RELATIONS: { source_document_id: string; target_document_id: string; relation_type: string; notes: string | null }[] = ${JSON.stringify(newRelations, null, 2)};
`;

  fs.writeFileSync(path.join(__dirname, '../src/lib/paco-data.ts'), outputCode);
  console.log('Saved to src/lib/paco-data.ts');
}
