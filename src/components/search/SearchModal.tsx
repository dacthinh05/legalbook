...
import type { ExternalLegalSearchResult } from '@/app/api/search/external/route';
...
export function SearchModal({
  ...
  // Scope state
  const [scopeFilter, setScopeFilter] = useState<'all' | 'document' | 'provision' | 'federated'>('all');

  // Federated Live External Search State
  const [externalResults, setExternalResults] = useState<ExternalLegalSearchResult[]>([]);
  const [isSearchingExternal, setIsSearchingExternal] = useState(false);
  const [isIngestingExternal, setIsIngestingExternal] = useState(false);
  const [ingestingItem, setIngestingItem] = useState<ExternalLegalSearchResult | null>(null);
  ...
  const handleSearchExternal = useCallback(async (q: string) => {
    const query = q.trim();
    if (!query || query.length < 2) return;
    setIsSearchingExternal(true);
    setScopeFilter('federated');
    try {
      ...
      console.warn('External federated search error:', err);
    } finally {
      setIsSearchingExternal(false);
    }
  }, []);

  const handleImportAndOpenExternal = useCallback(
    async (item: ExternalLegalSearchResult) => {
      if (item.isAvailableLocally && item.localDocumentId) {
        onSelectDocument(item.localDocumentId);
        onClose();
        return;
      }

      setIsIngestingExternal(true);
      setIngestingItem(item);
      ...
      try {
        ...
        console.error('Failed to ingest external document:', err);
      } finally {
        setIsIngestingExternal(false);
        setIngestingItem(null);
      }
    },
    ...
    try {
      const { results: res, scopeCounts: counts } = executeSearchWithScopeCounts(
        ...
        effectiveQuery,
        {
          typeFilter,
          statusFilter,
          scopeFilter: scopeFilter === 'federated' ? 'all' : scopeFilter,
          categoryDocIds,
          sortBy,
        }
      );
      ...
      >
          ...
          <div className="flex items-center justify-between gap-3 flex-wrap">
            ...
            <div className="flex items-center bg-slate-200/80 p-0.5 rounded-lg text-xs">
              ...
              <button
                ...
                onClick={() => {
                  setScopeFilter('federated');
                  if (externalResults.length === 0 && effectiveQuery) {
                    handleSearchExternal(effectiveQuery);
                  }
                }}
                className={`px-3 py-1.5 rounded-md font-semibold transition-all cursor-pointer flex items-center gap-1.5 ${
                  scopeFilter === 'federated'
                    ? 'bg-blue-600 text-white shadow-2xs'
                    : 'text-slate-600 hover:text-slate-900'
                }`}
                title="Tra cứu trực tiếp trên Cổng TTĐT Chính phủ & Tổng cục Thuế"
              >
                <Globe className="w-3.5 h-3.5 text-blue-300" />
                <span>Cổng Quốc gia & TVPL</span>
                {externalResults.length > 0 && (
                  <span className="px-1.5 py-0.2 bg-blue-700 text-white rounded text-[10px] font-mono">
        ...
        >
          {searchError ? (
            ...
            /* STATE FEDERATED: Live National Portal Results */
            <div className="p-3 sm:p-4 space-y-3">
              <div className="flex items-center justify-between gap-2 px-1 text-xs text-slate-500">
                <div className="flex items-center gap-1.5">
                  <Globe className="w-4 h-4 text-blue-600" />
                  <span className="font-semibold text-slate-800">
                    Kết quả trực tiếp từ Cổng TTĐT Chính phủ, Tổng cục Thuế & TVPL
                  </span>
                </div>
                {isSearchingExternal && (
                  <div className="flex items-center gap-1 text-blue-600 font-medium">
                    <Loader2 className="w-3.5 h-3.5 animate-spin" />
                    <span>Đang kết nối cổng quốc gia...</span>
                  </div>
                )}
              </div>

              {externalResults.length === 0 && !isSearchingExternal ? (
                <div className="py-12 px-6 text-center space-y-3 bg-slate-50 rounded-xl border border-slate-200">
                  <Globe className="w-8 h-8 mx-auto text-slate-400" />
                  <p className="text-sm font-semibold text-slate-800">
                    Bấm nút bên dưới để tìm kiếm trực tiếp trên toàn bộ Cổng Quốc gia
                  </p>
                  <button
                    onClick={() => handleSearchExternal(effectiveQuery)}
                  ...
                  >
                    ...
                    <span>Tra cứu &ldquo;{effectiveQuery}&rdquo; trên Cổng Quốc gia & TVPL</span>
                  </button>
                </div>
              ) : (
                <div className="space-y-2">
                  {externalResults.map((item) => (
                    <div
                      key={item.id}
                      onClick={() => handleImportAndOpenExternal(item)}
                      className="p-3.5 bg-white hover:bg-blue-50/70 border border-slate-200 hover:border-blue-300 rounded-xl transition-all cursor-pointer shadow-2xs group flex flex-col gap-1.5"
                    >
                      <div className="flex items-center justify-between gap-2 flex-wrap">
                        <div className="flex items-center gap-2">
                          <span
                            className={`px-2 py-0.5 rounded text-[10.5px] font-bold ${
                              item.source === 'chinhphu'
                          ...
                          <span className="text-[11px] text-slate-500">· {item.issuingBody}</span>
                        </div>

                        <div className="flex items-center gap-2 shrink-0">
                          {item.isAvailableLocally ? (
                            <span className="px-2 py-0.5 rounded text-[10.5px] font-semibold bg-emerald-50 text-emerald-800 border border-emerald-200 flex items-center gap-1">
                              <CheckCircle2 className="w-3 h-3 text-emerald-600" />
                              Đã có trong kho
                            </span>
                          ) : (
                            <span className="px-2 py-0.5 rounded text-[10.5px] font-semibold bg-blue-50 text-blue-800 border border-blue-200 flex items-center gap-1">
                          ...
                          <a
                            ...
                            title="Mở nguồn gốc trên trình duyệt"
                          >
                      ...
                      <h4 className="text-xs sm:text-[13.5px] font-bold text-slate-900 group-hover:text-blue-700 leading-snug">
                        {item.title}
                      </h4>
                      <p className="text-[11.5px] text-slate-600 line-clamp-2 leading-relaxed bg-slate-50 p-2 rounded-lg border border-slate-100">
                        {item.summarySnippet}
                      </p>
                    </div>
                  ))}
            ...
            <div className="py-12 px-6 text-center space-y-4">
              ...
              <div className="space-y-1">
                <h4 className="text-sm font-bold text-slate-900">
                  Không tìm thấy trong kho nội bộ khớp với &ldquo;{effectiveQuery}&rdquo;
                </h4>
                <p className="text-xs text-slate-500 max-w-sm mx-auto">
                  Bạn có thể tra cứu trực tiếp trên Cổng TTĐT Chính phủ, Tổng cục Thuế & TVPL:
                </p>
              </div>
              ...
              <div className="pt-2">
                <button
                  type="button"
                  onClick={() => handleSearchExternal(effectiveQuery)}
                  className="px-4 py-2.5 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl text-xs shadow-md inline-flex items-center gap-2 transition-all cursor-pointer"
                >
                  <Globe className="w-4 h-4 text-blue-200" />
                  <span>Tra cứu trên Cổng TTĐT Quốc gia & Tổng cục Thuế (1-Click)</span>
                </button>
              </div>
              ...
              {activeFiltersCount > 0 && (
                <div>
                  <button
                    onClick={handleClearFilters}
                    className="px-3 py-1.5 text-slate-600 hover:text-slate-900 font-medium rounded-lg text-xs transition-colors cursor-pointer"
                  >
                    Xóa các bộ lọc đang chọn ({activeFiltersCount})
                  </button>
                </div>
              )}
            </div>
          ...
          <div className="absolute inset-0 z-50 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4 animate-in fade-in duration-150">
            <div className="bg-white rounded-2xl p-6 max-w-md w-full shadow-2xl border border-slate-200 text-center space-y-4">
              ...
              <div className="space-y-1.5">
                <h4 className="text-sm font-bold text-slate-900">
                  Đang nạp & chuẩn hóa toàn văn {ingestingItem?.documentNumber || 'văn bản'}
                </h4>
                <p className="text-xs text-slate-600 leading-relaxed">
                  Hệ thống đang kết nối nguồn dữ liệu gốc, bóc tách cấu trúc Điều/Khoản và định dạng theo chuẩn Nghị định 30/2020/NĐ-CP...
                </p>
              </div>
              <div className="pt-2 text-[11px] text-blue-700 font-medium bg-blue-50 py-1.5 px-3 rounded-lg border border-blue-100 flex items-center justify-center gap-1.5">
                <Sparkles className="w-3.5 h-3.5" />
                <span>Chuẩn bị mở trong Reader...</span>
              </div>
            </div>
          </div>
        )}
