...
export function PointInTimeSelector({
  ...
  totalEffectsCount,
  compact = false,
}: PointInTimeSelectorProps & { compact?: boolean }) {
    ...
    return (
      <div className="flex items-center gap-1.5 text-[11px] text-slate-600 shrink-0">
        <div className="flex items-center gap-1">
          ...
          <span className="hidden xl:inline text-slate-500">Hiệu lực:</span>
        </div>
        ...
        <div className="flex items-center bg-white border border-slate-200 rounded p-0.5 shadow-2xs">
          <button
            ...
            className={cn(
              'px-1.5 py-0.2 rounded text-[10.5px] font-medium transition-colors cursor-pointer',
              isCurrent && !showCustomDate
                ? 'bg-blue-50 text-blue-900 font-bold'
                : 'text-slate-600 hover:text-slate-900'
            )}
            title={`Nội dung hiệu lực hiện hành (${formatDate(todayStr)})`}
          >
            Hiện hành
          </button>
          ...
          {issuedDate && (
            <button
              ...
              className={cn(
                'px-1.5 py-0.2 rounded text-[10.5px] font-medium transition-colors cursor-pointer hidden sm:inline-block',
                isIssued && !showCustomDate
                  ? 'bg-blue-50 text-blue-900 font-bold'
                  : 'text-slate-600 hover:text-slate-900'
              )}
              title={`Nội dung tại ngày ban hành (${formatDate(issuedDate)})`}
            >
              Gốc
            </button>
          )}
          ...
          <button
            ...
            className={cn(
              'px-1.5 py-0.2 rounded text-[10.5px] font-medium transition-colors cursor-pointer flex items-center gap-0.5',
              showCustomDate || (!isCurrent && !isIssued)
                ? 'bg-blue-50 text-blue-900 font-bold'
                : 'text-slate-600 hover:text-slate-900'
            )}
            title="Chọn ngày cụ thể để xem hiệu lực lịch sử"
          >
        ...
        {showCustomDate && (
          <input
            ...
            className="h-5 px-1 text-[10.5px] bg-white border border-blue-400 rounded outline-none text-slate-900 font-mono shadow-xs"
            aria-label="Chọn ngày tra cứu hiệu lực"
          />
        )}

        {/* Active changes toggle */}
        {totalEffectsCount > 0 && (
          <button
            type="button"
            onClick={onToggleShowOverlay}
            className={cn(
              'px-1.5 py-0.5 rounded border text-[10px] font-medium flex items-center gap-1 transition-colors cursor-pointer',
              showOverlay
          ...
          >
            {showOverlay ? <Eye className="w-3 h-3 text-blue-600" /> : <EyeOff className="w-3 h-3" />}
            <span className="hidden lg:inline">{activeEffectsCount} tác động</span>
          </button>
        )}
        ...
        <div className="relative">
          <button
            ...
            className="p-0.5 text-slate-400 hover:text-slate-700 rounded hover:bg-slate-100 transition-colors cursor-pointer flex items-center"
